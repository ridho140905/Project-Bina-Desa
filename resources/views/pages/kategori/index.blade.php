            ...
            <div class="card-body-universal">
                ...
                <form method="GET" action="{{ route('kategoriberita.index') }}" class="mb-4">
                    <div class="row g-3">
                        {{-- Filter Slug --}}
                        <div class="col-md-6">
                            <label class="form-label-universal">Filter Slug</label>
                            <div class="filter-container">
                                <select name="slug" class="form-select-universal" onchange="this.form.submit()">
                                    <option value="">Semua Slug</option>
                                    @foreach($allSlugs as $slugItem)
                                        <option value="{{ $slugItem->slug }}"
                                            {{ request('slug') == $slugItem->slug ? 'selected' : '' }}>
                                            {{ $slugItem->slug }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        {{-- Search --}}
                        <div class="col-md-6">
                            <label class="form-label-universal">Cari Kategori</label>
                            <div class="search-container">
                                <div class="input-group-universal">
                                    <input type="text" name="search" class="form-control-universal"
                                           value="{{ request('search') }}" placeholder="Cari nama atau deskripsi kategori...">
                ...
                {{-- Info Filter Aktif --}}
                @if(request('search') || request('slug'))
                            ...
                            @forelse($dataKategori as $item)
                                <tr>
                                    <td class="text-center text-muted-universal">
                                        {{ ($dataKategori->currentPage() - 1) * $dataKategori->perPage() + $loop->iteration }}
                                    </td>
                                    ...
                                    <td class="text-left">
                                        <div class="action-buttons">
                                            ...
                                            <form action="{{ route('kategoriberita.destroy', $item->kategori_id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-delete" onclick="return confirm('Yakin ingin menghapus kategori ini?')" title="Hapus Kategori">
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="empty-state-universal">
                                        ...
                                        @if(request('search') || request('slug'))
                                            Data kategori tidak ditemukan dengan filter yang dipilih.
                                        @else
                ...
                @if($dataKategori->hasPages())
                    <div class="mt-4">
                        <nav aria-label="Page navigation">
                            <ul class="pagination justify-content-center mb-0">
                                {{-- Previous Page Link --}}
                                @if ($dataKategori->onFirstPage())
                                ...
                                @else
                                    <li class="page-item">
                                        <a class="page-link page-link-arrow" href="{{ $dataKategori->previousPageUrl() }}">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z"/>
                                            </svg>
                                        </a>
                                ...
                                {{-- Pagination Elements --}}
                                @foreach ($dataKategori->getUrlRange(1, $dataKategori->lastPage()) as $page => $url)
                                    @if ($page == $dataKategori->currentPage())
                                ...
                                {{-- Next Page Link --}}
                                @if ($dataKategori->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link page-link-arrow" href="{{ $dataKategori->nextPageUrl() }}">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
